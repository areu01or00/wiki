...
def is_junk(url):
    ...
    return any(s in url for s in junk_hosts)
...
def main():
    ...
    for prof_name, prof_cfg in profiles.items():
        ...
        for r in results:
            ...
            n_new += 1
        print(f"  {prof_name}: {n_new} new (of {len(results)} results; skipped {profile_skipped_junk[prof_name]} junk, {profile_skipped_dup[prof_name]} dup)")
    ...
    for prof in profile_new_hashes:
        print(f"  profile {prof}: {len(profile_new_hashes[prof])} new hashes, {len(profile_new_topics.get(prof, []))} sub-topics")

    if total_new == 0:
        print("No updates needed.")
        # still update last_run and digest
    else:
        # update wiki pages
        for wiki_page_rel, entries in new_by_page.items():
            if wiki_page_rel.startswith("entities/"):
